...
방 예약 시스템을 붙일 예정이다. 아직 안 만들었고 명세만 있다 →
[docs/예약-시스템-명세.md](docs/예약-시스템-명세.md)

한 가지는 미리 해뒀다. **예약은 방을 `rooms.json` 의 `id`(`동-층F-이름`)로 가리키면
안 된다.** 이름이나 동이 바뀌면 그대로 깨진다 — 4동을 나눈 날 하루에만 26개 방의 id 가
바뀌었다. 그래서 파서가 Sweet Home 3D 의 방 UUID 를 `sourceId` 로 같이 내보낸다.
이름을 바꿔도 그대로 남는 값이다 (자세한 이유는 명세서에).
