...
- **조작은 한 패널에** — 검색 · 동 · 종류 · 시점 · 층이 왼쪽 패널 하나에 모여 있다.
  제목 옆 버튼으로 접으면 지도만 남는다
- **시점 프리셋** — `기본` / `평면` / `정면`. 방향만 정해두고 거리는 그때 보이는
...
- **좁은 화면** — 720px 이하에서 조작 패널이 아래 시트로 내려가고(엄지가 닿는 곳),
  방 정보 패널은 반대로 위로 올라간다. 둘 다 아래에 두면 서로 가린다.
  폰에서는 지도부터 보이도록 시트가 접힌 채로 시작하고, 조작 안내도 터치 기준으로 바뀐다
...
- **안 바뀌면 안 그린다.** 예전에는 가만히 둬도 초당 180장씩 다시 그렸다. 삼각형은
  6천 개뿐이라 형상은 안 무겁지만 **반투명한 면 445개를 겹쳐 칠하고 블룸까지 도는**
  그림이라 한 장이 싸지 않다. 폰에서는 그게 곧 발열이고, 발열이 오면 기기가 스스로
  속도를 낮춰 정작 움직일 때 더 끊긴다. 지금은 카메라·애니메이션·필터·선택이 바뀔
  때만 그린다 — 가만히 두면 그리기 호출 0회다.
- **약한 기기는 해상도를 낮춘다.** 비용이 전부 화면 픽셀 수에 비례하므로(오버드로,
  블룸) 가장 확실한 처방이다. 폰·터치 기기·코어 4개 이하면 픽셀비율 1.25,
  MSAA 2, 블룸을 절반 해상도로 흐린다(비용 1/4). 흐린 그림이라 티가 잘 안 난다.
...
예약 가능한 방(지금 13개)을 클릭하면 패널에 **예약하기** 버튼이 뜬다. 교시별 또는
시간별로 고르고 신청하면 담당 선생님이 승인해야 확정된다.

**화면은 다 됐고 서버만 붙이면 된다.** `src/config.js` 의 `RESERVATION_ENDPOINT` 가
비어 있는 동안에는 예약 창이 열리되 "아직 연결되지 않았습니다" 를 띄우고 신청 버튼이
잠긴다. 붙이는 법은 [tools/apps-script/README.md](tools/apps-script/README.md) 에
있고 10분이면 된다 — 구글 시트에 코드를 붙여넣고 웹앱으로 배포한 주소를 적으면 끝이다.

**관리자 화면은 만들지 않았다.** 선생님이 시트에서 `상태` 칸을 `대기` → `승인` 으로
바꾸는 게 곧 승인이다. 그래서 관리자 로그인도 권한 시스템도 필요 없다.

설계와 남은 결정은 [docs/예약-시스템-명세.md](docs/예약-시스템-명세.md) 에 있고,
무엇을 왜 그렇게 정했는지는 [docs/개발-일지.md](docs/개발-일지.md) 에 있다.

| 무엇을 고치려면 | 어디를 |
|---|---|
| 예약 받을 방 | `tools/plan.config.json` 의 `reservable` → 파서 재실행 |
| 교시 시간표 | `src/config.js` 의 `PERIODS` |
| 서버 주소 | `src/config.js` 의 `RESERVATION_ENDPOINT` |

> **예약은 방을 `rooms.json` 의 `id`(`동-층F-이름`)로 가리키지 않는다.** 이름이나 동이
> 바뀌면 그대로 깨지기 때문이다 — 4동을 나눈 날 하루에만 26개 방의 id 가 바뀌었다.
> 대신 파서가 내보내는 `sourceId`(Sweet Home 3D 의 방 UUID)를 쓴다. 실제로 `432` 와
> `433` 의 이름을 서로 맞바꿨을 때도 UUID 는 그대로여서 설정을 고칠 필요가 없었다.
...
- **파서 경고 0건.** 겹쳐 있던 `1층 홀` 라벨을 정리해서 라벨 180개 = 방 180개로 맞았다
