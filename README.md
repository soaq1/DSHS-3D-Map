...
현재 4개 층 184개 방을 담고 있다. (도면 기준일 2026-07-28)
...
- **홀은 동과 무관하게 한 색** — 세 동을 잇는 공용 공간이라 동 색을 따르면 소속이 있는 것처럼
  읽힌다. 슬레이트(`#94a3b8`) 한 색으로 묶는다. `plan.config.json` 의 `typeColors` 에서 바꾼다
...
**184개 방 전부 이름이 붙었고 파서 경고는 0건이다.** 방이 없는 라벨도, 라벨이 없는
방도 없다.
...
- 같은 층에 이름이 같은 방들이 있다 (`1층 홀` ×3, `3층 홀` ×2, `글로벌 라운지` ×2,
  `꿈돋움`·`꿈키움 라운지` ×2, `창고`·`화장실`). 뷰어가 검색에서 한 줄로 묶어 처리한다

최종 도면이 확정되면 `plan.config.json` 의 `expect` 에 층별 기댓값(방 개수·총 면적)을
적어둘 차례다. 그때부터는 도면이 틀어지면 파서가 바로 알려준다. 지금 적어두면 방을
하나 고칠 때마다 경고가 떠서 비워둔 상태다.
